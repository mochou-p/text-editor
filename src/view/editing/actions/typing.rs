...
impl super::super::Editing {
    pub fn newline(&mut self, editor: &mut Editor) {
    ...
    pub fn tab(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
        file.clean     = false;
    ...
    pub fn character(&mut self, editor: &mut Editor, ch: char) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
        file.clean     = false;
    ...
    pub fn erase_left(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
    ...
    pub fn erase_right(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
    ...
    pub fn move_line_up(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
    ...
    pub fn move_line_down(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
    ...
    pub fn erase_prev_word(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();

        for (i, cursor) in file.cursors.iter_mut().enumerate() {
            if cursor.x == 0 {
                self.erase_left(editor);
                return;
            }
    ...
    pub fn erase_next_word(&mut self, editor: &mut Editor) {
        let Some(file) = self.file.as_ref() else { return; };
        let      file  = editor.files.get_mut(file).unwrap();
